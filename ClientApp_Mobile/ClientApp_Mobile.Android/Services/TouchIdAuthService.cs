...
namespace ClientApp_Mobile.Droid.Services
{
    public class TouchIdAuthService : IBiometricPieAuthenticate
    {
        ...
        private const string REPLAY_ID = "68293";//Set random value?
        ...
        private void ShowBiometricPrompt(Java.Security.Signature signature)
        {
            ...
            biometricPrompt = new BiometricPrompt.Builder(activity)
                .SetDescription("")
                .SetTitle("Биометрическая аунтификация")
                .SetSubtitle("Дайте разрешение на биометрическую аутентификацию B2B Market")
                .SetNegativeButton("Отмена", activity.MainExecutor, negativeButtonListener)
                .Build();
            ...
            var callback = new BiometricAuthenticationCallback
            {
                Success = (BiometricPrompt.AuthenticationResult result) => {
                    ...
                    try
                    {
                        ...
                        MessagingCenter.Send<object>("AndroidAuth", "Success");
                    }
                ...
                Failed = () => {
                    MessagingCenter.Send<object>("AndroidAuth", "Fail");
                    //  Show error.
                },
